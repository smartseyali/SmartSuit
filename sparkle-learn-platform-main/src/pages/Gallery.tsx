...
const Gallery = () => {
  ...
  const renderMediaPreview = (item: GalleryItem, className = "") => {
    if (item.mediaType === 'video') {
      ...
      if (ytId) {
        return <img src={`https://img.youtube.com/vi/${ytId}/hqdefault.jpg`} alt={item.title} title={item.title} className={className} />;
      }
    ...
    return <img src={item.mediaUrl} alt={item.title} title={item.title} className={className} />;
  };
          ...
          >
            {item.mediaType === 'video' ? (
              <div className="relative">
                {ytId ? (
                  <img src={`https://img.youtube.com/vi/${ytId}/hqdefault.jpg`} className="w-full object-cover transition-transform duration-700 group-hover:scale-105" alt={item.title} title={item.title} />
                ) : (
              ...
              <img src={item.mediaUrl} alt={item.title} title={item.title} className="w-full object-cover transition-transform duration-700 group-hover:scale-105" />
            )}
